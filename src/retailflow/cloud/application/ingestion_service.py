...
# Decoupled from Pub/Sub parsing and HTTP event details.
...
class IngestionApplicationService:
    ...
    def __init__(self, container: IngestionDependencyContainer) -> None:
        ...
        self.watermark_service = container.watermark_service
        self.publisher_service = container.publisher_service
    ...
    def process_file_upload(self, gcs_event: GcsNotificationPayload, correlation_id: str) -> dict[str, Any]:
        ...
        # 2. Duplicate Detection (Task 2.3)
        if self.watermark_service.is_duplicate_hash(mock_file_hash):
            logger.warning(
        ...
        # 3. Register Watermark Ingestion (Task 2.3)
        self.watermark_service.register_ingestion(mock_file_hash, gcs_event.name, run_id)
        ...
        self.publisher_service.publish_accepted_event(accepted_event)
